// src/controllers/auth/merchant.auth.js
import mongoose  from 'mongoose';
import crypto    from 'crypto';
import Merchant  from '../../models/user/merchant.role.js';
...
export async function signUpMerchant(req, res, next) {
  const session = await mongoose.startSession();
  session.startTransaction();

  try {
    const { firstName, lastName, email, phone, password, city, businessName, businessType, businessAddress, cacNumber } = req.body;
    ...
    res.status(201).json({
      success: true,
      emailSent,
      message: 'Merchant account created. Check your email to verify.',
...
// Validation: merchantLoginSchema applied on route
export async function signInMerchant(req, res, next) {
  try {
    ...
    //2. Deactivated - admin explictly disabled this account 
    if (!user.isActive)
      return res.status(403).json({ success: false, message: 'Account is deactivated', deactivated: true });

    // Not yet is will be set up for version 2 
    // if (user.store && !user.store.isActive)
    //   return res.status(403).json({ success: false, message: 'Your store has been deactivated. Contact support.', storeInactive: true });
    ...
    // 4. Email verification checks - AFTER password is confirmed correct
